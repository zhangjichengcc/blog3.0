...
function deepClone(target) {
  ...
  for (const i in target) {
    cloneTarget[i] = typeof target[i] === 'object' ? deepClone(target[i]) : target[i];
  }
  return cloneTarget;
}
...
const FormItem: FC<formItemProps> = (props) => {
  ...
  return (
    <div className={classnames(status === 'edit' && styles.formItem, device === 'pc' ? styles.pcItem : '')}>
      <input type="text" id={schema.key} style={{ opacity: 0, width: 0, height: 0, position: 'absolute', top: '50%', left: -10 }} />
      <Com {...otherProps} />
    </div>
  );
}
    ...
    <>
      {
        propertiesList.length > 0 ? (
          <div className={styles.formRender} style={style}>
            <Form>  
              <ReactSortable list={propertiesList} setList={setList} animation={150} disabled={status !== 'edit'}>
                {
                  propertiesList.map(item => {
                    ...
                    // 问卷已填
                    const isAnswered = answerContent ? Object.keys(answerContent).length : 0 
