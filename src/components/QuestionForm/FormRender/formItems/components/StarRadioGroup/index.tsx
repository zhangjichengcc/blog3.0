...
const StarRadioGroup: FC<startRadioGroupProps> = ({
  ...
  const activeIdx = data.map(item => item.toString()).indexOf(coreValue || value);
        ...
        data.map((item, idx) => {
          ...
          return (
            <div className={styles.item} onClick={() => onHandleClick(key)}>
              { active ? 
                <Icon className={disabled ? styles.fillGrayStar: styles.fillStar} type="star" theme="filled" /> :
                <Icon className={disabled ? styles.disableStar : styles.star} type="star" />
              }
            </div>
          )
        })
