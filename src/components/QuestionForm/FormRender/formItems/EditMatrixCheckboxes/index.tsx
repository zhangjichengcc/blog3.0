 ...
 * @LastEditTime: 2021-05-21 14:43:36
 * @LastEditors: zhangjicheng
 * @Description: 二次封装单行输入框
 * @FilePath: \wechat-revenue-g\src\components\QuestionForm\FormRender\formItems\EditMatrixCheckboxes\index.tsx
...
interface editMatrixCheckboxesProps {
  value?: object;
  name?: string;
...
const EditMatrixCheckboxes: FC<editMatrixCheckboxesProps> = ({
  value = {},
  name,
  status,
  device = 'pc',
                          ...
                          <Checkbox.Group>
                            {
                              answersRenderEnumList.map(ansItem => (
                                <Checkbox value={ansItem.id} className={styles.questionItem} style={{width: `${100/answersRenderEnumList.length}%`}} />
                              ))
                            }
                          </Checkbox.Group>
                        </div>
                ...
                <div className={styles.questionBody}>
                  ...
                  </div>
                  {
                    questionsRenderEnumList.map(qusItem => (
                      <div className={styles.questionGroup}>
                        <span>{qusItem.value}</span>
                        <Checkbox.Group value={value[qusItem.id]} disabled={disabled} onChange={e => onHandleChange(e, qusItem)}>
                          {
                            answersRenderEnumList.map(ansItem => (
                              <Checkbox value={ansItem.id} className={styles.questionItem} style={{width: `${100/answersRenderEnumList.length}%`}} />
                            ))
...
const Component = Form.create()(EditMatrixCheckboxes);
