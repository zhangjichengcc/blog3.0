 ...
 * @LastEditTime: 2021-05-21 14:41:21
 * @LastEditors: zhangjicheng
 * @Description: 二次封装单行输入框
 * @FilePath: \wechat-revenue-g\src\components\QuestionForm\FormRender\formItems\EditRadio\index.tsx
            ...
            { edit ?
              ...
              <div className={styles.editForm}>
                ...
                <Form.Item label="" style={{marginLeft: 63}}>
                  ...
                  })(<InputSortArea style={{ marginLeft: 40 }} />)}
                </Form.Item>
                ...
                <div className={styles.questionArea}>
                  <span className={styles.label}>
                    {/* {(description && description.length) && 
                      <Popover content={description} title="" trigger="hover">
                        <Icon type="info-circle" theme="filled" style={{color: '#1890FF'}} />
                      </Popover>
                    } */}
                    ...
                    <Radio.Group style={{display: 'block'}}>
                      {
                        renderEnumList.map(item => (
                          <Radio value={item.id} className={styles.questionItem}>
                            {item.value}
                          </Radio>
                        ))
                      }
          ...
          <div className={styles.preview}>
            <div className={styles.showForm}>
              <div className={styles.questionArea}>
                ...
                {(description && description.length) && <span className={styles.description}><Icon type="info-circle" style={{paddingRight: 5, fontSize: 14}} />{description}</span>}
                <div className={styles.questionBody}>
                  <Radio.Group style={{display: 'block'}} disabled={disabled} value={value} onChange={onHandleChange}>
                    {
                      renderEnumList.map(item => (
                        <Radio value={item.id} className={styles.questionItem}>
                          {item.value}
