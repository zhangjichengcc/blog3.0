...
const calcStrLen = str => {
  ...
  str.split('').forEach((_item, idx) => {
    len = str.charCodeAt(idx) > 0 && str.charCodeAt(idx) < 128 ? len + 1 : len + 2;
  });
  return len;
};
...
const displayData = (data, title = '', root = 'root', impPosition) => {
  ...
  // 定义根目录
  const res = { label: rootLabel, id: root, children: [], level: 0, rootTree: root === 'root' };
  // 格式化子元素
  ...
  const displayChildren = (prefix, key, position, all = false) => {
    ...
    const childArrDef = arr.map((item, idx) => {
      ...
      return {
        ...item,
        id: `${prefix}_${idx}`,
        label: calcStrLen(nsrmc) > labelMaxLength ? `${nsrmc.slice(0, labelMaxLength)}...` : nsrmc,
        qlabel: nsrmc,
        level: 2,
...
class EnterPriseMap extends Component {
  ...
  initCharts = () => {
    ...
    // 节点设置
    graph.node(node => {
      const { label = '', color = null, type, position, id, level, cgbl, children } = node;
      const nodeOpt = {
  ...
  loadMore = child => {
    ...
    // parent.lock();
    // eslint-disable-next-line no-underscore-dangle
    const { _children: children, pageSize, pageNum, id: pid } = parent.getModel();
  ...
  chooseLink = key => {
    ...
    targetData.children = selectedLink.includes(key) ? [] : _children.filter(v => v.name === key);
    graph.refreshLayout();
  ...
  render() {
    ...
    const { qlabel: modelTitle = '企业信息', selectedLink = [] } = selectedModel;
    return (
      <div className={styles.pageView}>
        <div id="mountNode_enterprise_map" style={{ width: '100%', height: '100%' }} />
        <Drawer
          title={modelTitle}
          placement="left"
          closable={false}
