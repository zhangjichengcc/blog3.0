...
export default class OperationModal extends PureComponent {
  ...
  treeRender = data => {
    return data.map(item => {
      ...
      } else if (item.type === 'folder' || (!item.type && item.operation === '0')) {
  ...
  newFolderRender = () => {
    ...
    return (
      <span>
        ...
        />
        <Button className={styles.newFolderBtn} icon="check" size="small" onClick={() => this.editFolderNameOk()} />
        <Button className={styles.newFolderBtn} icon="close" size="small" onClick={() => this.editFolderNameCancel()} />
      </span>
    )
  }
  ...
  editFolderNameOk = () => {
    ...
    loop(list, 'newId', (item, index, arr) => {
      ...
      if (isSame) {
        ...
        addNewFolderNameOk({dirName: newFolderName, parentId: Number(expandedKeys[0])});
      }
    });
  }
  ...
  render () {
    const { selectTreeNode, expandedKeys } = this.state;
    const { title, handleOk, fileList, visible, loading, addBtnStatus = true } = this.props;
    ...
    return (
      ...
      >
        ...
        {addBtnStatus && <Button className={styles.addBtn} onClick={() => {this.addNewFolder()}}>新建文件夹</Button>}
      </Modal>
    )
  }
}