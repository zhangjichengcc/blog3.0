...
const JsrCom: FC<jsrComProps> = (props = {}) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center" }}>
      ...
      />
      <span
        style={{ width: 100, fontSize: 13, paddingLeft: 36, cursor: "pointer" }}
        onClick={() => onHandleClick(!showBar)}
      >
        {showBar ? "收起群组" : "展开群组"}
      </span>
    </div>
...
const QuestionBaseInfo: FC<QuestionBaseInfoProps & FormComponentProps> = ({
  ...
  function selectNsr(obj) {
    ...
    const newList = checked
      ? [...yxNsrList, obj]
      : yxNsrList.filter(item => item.key !== obj.key);
      ...
      >
        ...
        <Form.Item label="问卷标题">
          {getFieldDecorator("title", {
            rules: [
              { required: true, message: "请输入问卷标题", whitespace: true }
            ]
              ...
              { required: true },
              {
                validator: (_rule, val, callback) => {
                  if (val.status === "Y" && !val.date) {
                    ...
                    val.status === "Y" &&
                    moment(value.data).isBefore(moment(new Date()))
                  ) {
                    callback("日期不能早于当前时间！");
                  } else {
                    callback();
